...
define('PASSWORD', '7226658400a4e3332e59912440e7fc0dc527f8e0a8b4480fb61e235b7e50982352e12597451641f6402383aa9efc4ce8a458d78749b75b324fd256d5cbc8fd8a');
...
if (empty(PASSWORD) === false && (isset($_SESSION['dinesh_admin'], $_SESSION['dinesh_password']) === false || $_SESSION['dinesh_admin'] !== true || $_SESSION['dinesh_password'] != PASSWORD)) {
	if (isset($_POST['dinesh_password']) && empty($_POST['dinesh_password']) === false) {
		$password_hash = hash('sha512', $_POST['dinesh_password']);

		if ($password_hash === PASSWORD) {
			session_regenerate_id(true);

			$_SESSION['dinesh_admin'] = true;
			$_SESSION['dinesh_password'] = $password_hash;
	...
	die('<title>dinesh</title><form method="post"><div style="text-align:center"><h1><a href="http://github.com/dinesh/dinesh" target="_blank" title="PHP file editor" style="color:#444;text-decoration:none" tabindex="3">dinesh</a></h1>' . (isset($error) ? '<p style="color:#dd0000">' . $error . '</p>' : null) . '<input id="dinesh_password" name="dinesh_password" type="password" value="" placeholder="Password&hellip;" tabindex="1"><br><br><input type="submit" value="Login" tabindex="2"></div></form><script type="text/javascript">document.getElementById("dinesh_password").focus();</script>');
}

if (isset($_GET['logout'])) {
	if ($_GET['logout'] == $_SESSION['dinesh_token']) {
		unset($_SESSION['dinesh_admin']);
...
if (isset($_GET['path'])) {
	...
	$session_token = $_SESSION['dinesh_token'] ?? null;
...
$_SESSION['dinesh_token'] = bin2hex(random_bytes(32));
...
<body>
	<?php if (PASSWORD == hash('sha512', 'admin')) : ?>
		<div class="heading-alert alert alert-warning"><i class="fa fa-info-circle"></i><span class="ml-2">You are using dinesh with default password. Please click <a href="javascript:void(0);" class="change-password">here</a> to change password after installation.</span></div>
	<?php endif; ?>
	...
	<div class="container-fluid">
		...
		<div class="row p-3">
			<div class="col-md-3">
				<h1><a href="http://github.com/dinesh/dinesh" target="_blank" title="dinesh <?= VERSION ?>">dinesh</a></h1>
			</div>
			<div class="col-md-9">
				...
				<div class="float-right">
					...
					<?php if (in_array('changepassword', $permissions)) { ?><a href="javascript:void(0);" class="change-password btn btn-sm btn-primary"><i class="fas fa-key"></i></a> &nbsp; <?php } ?><a href="<?= $_SERVER['SCRIPT_NAME'] ?>?logout=<?= $_SESSION['dinesh_token'] ?>" class="btn btn-sm btn-danger"><i class="fas fa-sign-out-alt"></i></a>
				</div>
			</div>
		</div>
	...
	<form method="post">
		...
		<input name="token" type="hidden" value="<?= $_SESSION['dinesh_token'] ?>">
